...
serving-path purity and leak-soak checks."""
...
def test_huge_integer_epoch_is_bounded(db):
    ...
    overflow the timestamp buffer — beyond the 9999 boundary it degrades
    to a status document, never garbage."""
...
def test_serving_path_writes_nothing(db):
    """Receipts are gone: forecast and detect_anomalies are pure
    functions. A call creates zero _predict tables and works with the
    database pinned read-only."""
    ...
    n, = db.execute(
        "SELECT count(*) FROM sqlite_master WHERE name LIKE '\\_predict%'"
        " ESCAPE '\\'").fetchone()
    assert n == 0
...
def test_no_memory_growth_over_many_calls(db):
    ...
    def batch(n):
        for _ in range(n):
            db.execute(
                "SELECT forecast(ts, value, 5) FROM series").fetchall()
            db.execute(
...
@pytest.mark.parametrize("sql", [
    "SELECT forecast(ts, value, 3, ?) FROM series",
    "SELECT detect_anomalies(ts, value, ?) FROM series",
], ids=["forecast", "detect_anomalies"])
def test_duplicate_option_keys_no_leak_ts(db, sql):
    ...
    (raw,) = db.execute(sql, (dup,)).fetchone()
    doc = json.loads(raw)
    assert doc["model"] == "stub-seasonal-naive"  # last wins
